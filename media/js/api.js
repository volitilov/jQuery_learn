...
after(content)
// вставляет контент после каждого элемента из выборки, т.е. если
// вы встречаете строку $("p").after("<hr/>") , читайте её как 
...
append(content)
// вставляет контент в конец каждого элемента из выборки, т.е.
// строку кода $("p").append("<hr/>") , следует читать как 
...
wrap(element)
// оборачиваем каждый найденный элемент новым элементом, т.е.
// мы конфеты из коробки заворачиваем в фантики

wrapAll(element)
// оборачивает найденные элементы новым элементом, мы
// берём все конфеты, и заворачиваем в один большой фантик

wrapInner(element)
// оборачивает контент каждого найденного элемента новым
// элементом, берём конфеты, убираем фантики, заворачиваем в свой 
// фантик, и сверху заворачиваем в родной фантик

unwrap()
// удаляет родительский элемент у найденных элементов, фантики вон

clone(withDataAndEvents)
// клонирует выбранные элементы, для дальнейшей вставки копий 
// назад в DOM, позволяет так же копировать и обработчики событий

detach()
// удаляет элемент из DOM, но при этом сохраняет все данные 
// о нём в jQuery, следует использовать, если надо удалить 
// элемент, а потом вернуть его обратно

empty()
// удаляет текст и дочерние DOM элементы

remove()
// удаляет элемент из DOM, насовсем

html()
// вернёт HTML заданного элемента

html(newHtml)
// заменит HTML в заданном элементе

text()
// вернёт текст заданного элемента, если внутри элемента 
// будут другие HTML тэги, то вернётся сборная солянка из 
// текста всех элементов

text(newText)
// заменит текст внутри выбранных элементов, при попытке вставить
// таким образом HTML, будет получен текст, где тэги будут приведены 
// к HTML entities:

serialize()
// он в ответе за «сбор» данных с формы в удобном для передачи 
// данных формате: name=Ivan&role=Admin
...
offset()
// вернёт позицию DOM элемента относительно document'а, данные будут
// получены в виде объекта: { top: 10, left: 30 }

offset({ top: 10, left: 30 })
// устанавливаем расположение DOM элемента по указанным координатам

position()
// вернёт позицию DOM элемента относительно родительского элемента

height()
// возвращает высоту элемента за вычетом отступов и границ; если у 
// нас несколько элементов в выборке, вернётся первый; значение, в 
// отличии от метода
...
height(height)
// устанавливает высоту всех элементов в выборке, если значение 
...
// вернут соответственно высоту и ширину элемента, включая padding

outerHeight() и outerWidth()
// вернут высоту и ширину элемента, включая padding и border

outerHeight(true) и outerWidth(true)
// высота и ширина, включая padding, border и margin

scrollLeft()
// возвращает значение «проскроленности» по горизонтали первого
// элемента из выборки

scrollLeft(value)
// устанавливает значение горизонтального скрола для каждого
// элемента из выборки

scrollTop()
// возвращает значение «проскроленности» по вертикали первого
// элемента из выборки

scrollTop(value)
// устанавливает значение вертикального скрола для каждого
//элемента из выборки
